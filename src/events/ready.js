...
module.exports = {
  async execute(client) {
    client.on("ready", () => {
      const activities = [
        { name: `${process.version} on ${process.platform} ${process.arch}`, type: 2 }, // LISTENING
        { name: `${os.cpus().length} Core`, type: 0 }, // PLAYING
        { name: `Speed ${core.speed}MHz `, type: 3 }, // WATCHING
        { name: `/help | @crux`, type: 5 } // COMPETING
      ];
      const status = [
        'online',
        'dnd',
